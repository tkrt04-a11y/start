...
def handle_collect(args: list[str]) -> None:
    """Collect information and persist it.

    ``args`` may contain ``[source, content]``; if elements are missing
    the user will be prompted interactively.  This helper is separated
    out so that it can be invoked directly from tests.
    """
    if len(args) >= 1:
        source = args[0]
    else:
...
def main():
    # command dispatching comes first so that the collector can be used

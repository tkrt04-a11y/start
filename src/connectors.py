...
def fetch_github_issues(repo: str, state: str = "open", limit: int = 20) -> list[dict[str, Any]]:
    """Fetch issues from GitHub and return normalized entries."""
...
def fetch_rss_feed(feed_url: str, limit: int = 20) -> list[dict[str, Any]]:
    """Fetch a simple RSS/Atom feed and normalize to entries.

    This intentionally uses lightweight parsing to keep dependencies small.
    """
    ...
    # Minimal parser by splitting item/entry blocks.
...
def fetch_survey_json(json_path: Path | str, content_field: str = "content") -> list[dict[str, Any]]:
    """Load survey results from a local JSON array file."""
    path = Path(json_path)

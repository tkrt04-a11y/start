...
function Test-AlertWebhookDispatch {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            Add-Utf8Lines -Path $logFile -Lines @("[$(Get-Date -Format s)] WARNING daily pipeline: alert dedup helper failed (exit=$LASTEXITCODE). webhook send will proceed")
            return $true
        }

        $dedupText = ($dedupOutput | Out-String).Trim()
        if (-not $dedupText) {
            Add-Utf8Lines -Path $logFile -Lines @("[$(Get-Date -Format s)] WARNING daily pipeline: alert dedup helper returned empty output. webhook send will proceed")
            return $true
        }
        ...
        Add-Utf8Lines -Path $logFile -Lines @("[$(Get-Date -Format s)] INFO daily pipeline: webhook duplicate suppressed within cooldown ($alertDedupCooldownSec sec, signature=$signatureText)")
        return $false
    } catch {
        Add-Utf8Lines -Path $logFile -Lines @("[$(Get-Date -Format s)] WARNING daily pipeline: alert dedup helper error: $($_.Exception.Message). webhook send will proceed")
        return $true
    }
}
...
function Write-Alert {
    ...
    if ($WebhookUrl -and $shouldSendWebhook) {
        ...
        for ($attempt = 1; $attempt -le $alertWebhookRetries; $attempt++) {
            try {
                Add-Utf8Lines -Path $logFile -Lines @("[$(Get-Date -Format s)] INFO daily pipeline: alert webhook attempt $attempt/$alertWebhookRetries")
                Invoke-RestMethod -Method Post -Uri $WebhookUrl -ContentType "application/json" -Body $payload | Out-Null
                Add-Utf8Lines -Path $logFile -Lines @("[$(Get-Date -Format s)] INFO daily pipeline: alert webhook sent (attempt $attempt/$alertWebhookRetries)")
                break
            } catch {
                if ($attempt -lt $alertWebhookRetries) {
                    $sleepSec = [Math]::Round($alertWebhookBackoffSec * [Math]::Pow(2, $attempt - 1), 3)
                    Add-Utf8Lines -Path $logFile -Lines @("[$(Get-Date -Format s)] WARNING daily pipeline: alert webhook attempt $attempt/$alertWebhookRetries failed: $($_.Exception.Message). retrying in $sleepSec sec")
                    Start-Sleep -Seconds $sleepSec
                } else {
                    Add-Utf8Lines -Path $logFile -Lines @("[$(Get-Date -Format s)] ERROR daily pipeline: alert webhook final failure after $alertWebhookRetries attempts: $($_.Exception.Message)")
                }
            }
        }
    }
...
$logFile = Join-Path $logDir ("daily-run-" + (Get-Date -Format "yyyyMMdd-HHmmss") + ".log")
...
$commands = @(
    "python -m src.main fetch rss https://hnrss.org/frontpage 15",
...
Add-Utf8Lines -Path $logFile -Lines @("=== Daily pipeline started: $(Get-Date -Format s) ===")
if ($alertWebhookFormatInvalid) {
    Add-Utf8Lines -Path $logFile -Lines @("[$(Get-Date -Format s)] WARNING daily pipeline: invalid ALERT_WEBHOOK_FORMAT '$alertWebhookFormatText'. using 'generic'")
}
if ($monthlyReportCommand) {
    $commands += $monthlyReportCommand
    $monthlyLine = "[$(Get-Date -Format s)] INFO daily pipeline: monthly report scheduled for $monthlyReportTarget"
    Add-Utf8Lines -Path $logFile -Lines @($monthlyLine)
    Add-Utf8Lines -Path $alertFile -Lines @($monthlyLine)
}
Add-Utf8Lines -Path $logFile -Lines @("[$(Get-Date -Format s)] INFO daily pipeline: alert webhook format=$alertWebhookFormat")
Add-Utf8Lines -Path $logFile -Lines @("[$(Get-Date -Format s)] INFO daily pipeline: alert dedup cooldown sec=$alertDedupCooldownSec state=$alertDedupStatePath")
$commandFailures = 0
...
foreach ($cmd in $commands) {
    ...
    try {
        ...
        $commandFailures += 1
        $failureLine = "[$(Get-Date -Format s)] ERROR daily pipeline: command failed: $cmd"
    ...
    if ($cmd -eq "python -m src.main apply-insights --ai") {
        ...
        if ($null -ne $promotedCount -and $promotedCount -lt $promotedMin) {
            $alertLine = "[$(Get-Date -Format s)] WARNING daily pipeline: promoted actions below threshold ($promotedCount < $promotedMin)"
    ...
    if ($cmd -eq "python -m src.main metrics-check --days 30" -and $cmdExitCode -ne 0) {
        $alertLine = "[$(Get-Date -Format s)] WARNING daily pipeline: metrics-check reported threshold violations"
        Write-Alert -Line $alertLine -WebhookUrl $env:ALERT_WEBHOOK_URL
        $runAlerts.Add($alertLine) | Out-Null
    }
}
Add-Utf8Lines -Path $logFile -Lines @("=== Daily pipeline finished: $(Get-Date -Format s) ===")
...
$metricsPath = Join-Path $logDir ("daily-metrics-" + (Get-Date -Format "yyyyMMdd-HHmmss") + ".json")
$metrics = [ordered]@{
    pipeline = "daily"
...
$summaryPath = Join-Path $logDir ("alerts-summary-" + (Get-Date -Format "yyyyMMdd") + ".md")
$summaryLines = @(
    "# Alert Summary (Daily)",
    "",

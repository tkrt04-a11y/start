...
def test_write_weekly_report(tmp_path):
    ...
    assert path.exists()
    latest = tmp_path / "latest_weekly_report.md"
...
def test_write_monthly_report(tmp_path):
    ...
    assert path.exists()
    latest = tmp_path / "latest_monthly_report.md"
    assert latest.exists()
...
def test_filter_entries_by_days():
    ...
    assert any(e.get("source") == "a" for e in filtered)
    assert any(e.get("source") == "legacy" for e in filtered)
